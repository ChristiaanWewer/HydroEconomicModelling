...
# add scenario edit
# Ep 2.4 percent higher
forcing2013[:,2] = forcing2013[:,2] * 1.024
# Pcp 3.4 percent lower
forcing2013[:,1] = forcing2013[:,1] * (1-0.034)
...
# parameters for the crops:  coffee   pepper
start_irrigating = np.array([0       ,90    ])  # day irrigation starts
end_irrigating =   np.array([-1      ,214   ])  # day irrigation ends
compo =            np.array([0.8     ,0.2   ])  # composition of crops, -
Ky =               np.array([1       ,1.1   ])  # yield response factor, -
Yo =               np.array([6000    ,26000 ])  # optimum yield / irrigation yield, kg/ha
profit =           np.array([85      ,47    ])  # profit for crops, RS/kg
area_Plateau = 21725
